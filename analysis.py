...
try:
    ...
    print(
        "Error: impedance_rs module not found. Please build and install it using 'maturin develop --release'."
    )
    sys.exit(1)
...
def main():
    ...
    try:
        ...
        (
            ...
            fit_mag,
            fit_phase,
        ) = impedance_rs.fit_circuit(  # type: ignore
            circuit_str,
            freq.tolist(),
            z_real_np.tolist(),
            z_imag_np.tolist(),
        ...
        ax2_mag.loglog(
            freq, np.sqrt(z_real_np**2 + z_imag_np**2), "o", label="|Z| Data"
        )
        ax2_mag.loglog(freq, fit_mag, "-", label="|Z| Fit")

        ax2_phase.semilogx(
            freq, phase, "s", color="orange", label="Phase Data", alpha=0.5
        )
        ...
        fformat.saveFig(fig, "EIS_Fitting")
